...
BEGIN
    ...
    -- VARIABLES: DEBUGGING
    -------------------------------------------------------------
    DECLARE @ProcedureName AS NVARCHAR(128) = N'dbo.spMFSetUniqueIndexes';
...
IF EXISTS(SELECT 1 FROM sys.default_constraints AS dc WHERE name = ''DF_' + @TableName + '_ObjID'' )
ALTER TABLE ' + QUOTENAME(@TableName) + N'
DROP CONSTRAINT DF_' + @TableName + '_ObjID;'
--ALTER TABLE ' + QUOTENAME(@TableName) + N' ADD  CONSTRAINT [DF_' + @TableName + N'_ObjID]  DEFAULT (IDENT_CURRENT('''
 --                 + @TableName + ''')*-1) FOR [ObjID]';
...
WHERE name=''IX_' + @TableName + N'_Objid'' AND object_id = OBJECT_ID(''dbo.' + @TableName
                  + N'''))
DROP index IX_' + @TableName + N'_Objid ON ' + @TableName + ';
CREATE NONCLUSTERED INDEX IX_' + @TableName + N'_Objid
