...
EXEC [setup].[spMFSQLObjectsControl] @SchemaName = N'dbo'
								   ...
								   , @Object_Release = '4.2.7.45'		   -- varchar(50)
								   , @UpdateFlag = 2;
-- smallint
GO
/*------------------------------------------------------------------------------------------------
          ...
          -----------------------------------------------------------------------
            IF OBJECT_ID('tempdb.dbo.#NewLoginAccountTble') IS NOT NULL
                BEGIN
                    ...
                    FROM    MFLoginAccount
                            INNER JOIN #NewLoginAccountTble ON MFLoginAccount.UserName = #NewLoginAccountTble.UserName;
...
EXEC Setup.[spMFSQLObjectsControl] @SchemaName = N'dbo',
    ...
    @Object_Release = '3.1.2.40', -- varchar(50)
    @UpdateFlag = 2;
 -- smallint
...
EXEC [setup].[spMFSQLObjectsControl] @SchemaName = N'dbo',
                                     ...
                                     @Object_Release = '4.2.6.44',             -- varchar(50)
                                     @UpdateFlag = 2;                          -- smallint
...
ALTER PROC [dbo].[spMFVaultConnectionTest] @MessageOut NVARCHAR(100) = NULL OUTPUT
AS
BEGIN
    SET NOCOUNT ON;
    /*
...
*/
    --DECLARE @messageOut VARCHAR(100)
    DECLARE @Return_Value INT;
...
SELECT @vaultsettings = [dbo].[FnMFVaultSettings]()
	...
	BEGIN CATCH
        ...
        SET @MessageOut = 'Vault connection failed, check the settings and update with spMFSettingsForVaultUpdate';
...
ALTER PROCEDURE [dbo].[spMFSearchForObject] (@ClassID     INT
                                              ...
                                              ,@Count      INT = 1
											  ,@Debug SMALLINT = 0
											  ,@OutputType INT = 0 -- 0 = output to select 1 = output to search table
...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            DECLARE @ProcedureStep VARCHAR(100)
            ...
            SELECT @ProcedureStep = 'Drop and synchronize metadata';
            ...
            EXEC [dbo].[spMFCheckLicenseStatus] @InternalProcedureName = 'spMFGetClass' -- nvarchar(500)
                                               ...
                                               ,@ProcedureStep = @ProcedureStep;        -- sysname
            ...
            --Delete the data from Metadata Table
            ...
            IF @Debug > 0
                RAISERROR('%s : Step %s ReturnValue: %i', 10, 1, @ProcedureName, @ProcedureStep, @return_value);

            --IF synchronize is success
            IF (@return_value = 1 AND @IsReset = 0)
            BEGIN
                SET @ProcedureStep = 'Update with no reset';

                IF @Debug > 0
                    RAISERROR('%s : Step %s IsReset: %i', 10, 1, @ProcedureName, @ProcedureStep, @IsReset);
                ...
                FROM [dbo].[MFProperty]          [MP]
                    INNER JOIN [#MFPropertyTemp] AS [tmp]
                        ON [MP].[MFID] = [tmp].[MFID]
                WHERE [MP].[MFDataType_ID] != [tmp].[MFDataType_ID];

                IF EXISTS (SELECT * FROM [#TempProperty])
                BEGIN
                    IF @WithClassTableReset = 0
                    BEGIN
                        ...
                        RAISERROR(
                                    ...
                                    ,'datatype of property has changed'
                                    ,@ErrMsg
                                 );
                    END;
                    ELSE
            ...
            ELSE
            BEGIN
                SET @ProcedureStep = 'Update with reset';
            ...
            -------------------------------------------------------------
            -- New feature - auto add columns in class table 
            -------------------------------------------------------------

            /*
check table before update and auto create any columns
--check existence of table
--drop and update metadata structure only
--get missing columns in table
--add new columns to existing table (preventing having to drop and recreate table
...
*/
            DECLARE @SQLDataType NVARCHAR(200);
            DECLARE @Datatype INT;
            DECLARE @Property NVARCHAR(100);
            DECLARE @rownr INT;
            DECLARE @SQL NVARCHAR(MAX);

            DECLARE @MisssingColumns AS TABLE
            (
                [id] INT IDENTITY
            ...
            FROM [dbo].[MFvwMetadataStructure] AS [mfms]
                INNER JOIN [dbo].[MFClass]     AS [mc]
                    ON [mfms].[class_MFID] = [mc].[MFID]
            WHERE [mc].[IncludeInApp] IS NOT NULL
            EXCEPT
            SELECT [Tablename]
                  ,[property]
            FROM [mfvwclasstablecolumns];

            UPDATE @MisssingColumns
            SET [ColumnName] = [mp].[ColumnName]
               ,[Datatype] = [mp].[MFDataType_ID]
            FROM @MisssingColumns             AS [mc]
                INNER JOIN [dbo].[MFProperty] AS [mp]
                    ON [mp].[Name] = [mc].[PropertyName];
            ...
            BEGIN
                SELECT @MFTableName = [mc].[Tablename]
                      ...
                      ,@Property    = [mc].[PropertyName]
                FROM @MisssingColumns             AS [mc]
                    INNER JOIN [dbo].[MFDataType] AS [mdt]
                        ON [mc].[Datatype] = [mdt].[MFTypeID]
                WHERE [mc].[id] = @rownr;

                IF @Datatype = 9
                BEGIN
                    SET @SQL = N'Alter table ' + QUOTENAME(@MFTableName) + ' Add ' + @Property + ' Nvarchar(100);';

                    EXEC [sys].[sp_executesql] @SQL;

                    PRINT '##### ' + @Property + ' added for table ' + QUOTENAME(@MFTableName) + '';
                END;

                IF @Datatype = 10
                BEGIN
                    SET @SQL = N'Alter table ' + QUOTENAME(@MFTableName) + ' Add ' + @Property + ' Nvarchar(4000);';
                ...
                SET @SQL
                    = N'Alter table ' + QUOTENAME(@MFTableName) + ' Add ' + @ColumnName + ' ' + @SQLDataType + ';';
...
EXEC [setup].[spMFSQLObjectsControl] @SchemaName = N'dbo',
                                     ...
                                     @Object_Release = '4.1.7.46',             -- varchar(50)
                                     @UpdateFlag = 2;                          -- smallint
...
*************************************************************************************/
	...
	-- SET @trancount = @@TranCount
	 ...
	 BEGIN try   
	    IF @trancount = 0
	--	BEGIN TRAN
                IF @ProcessBatch_ID IS NULL AND  @DetailLoggingIsActive = 1
                Begin
				...
				[LogType] = CASE
                   ...
                   , [LogText] = CASE
                                    ...
                                    ELSE
                                        @LogText
                                END,
                    [Status] = CASE WHEN @logstatus IS NULL THEN 'Completed'
					else
					@LogStatus end
...
PRINT SPACE(5) + QUOTENAME(@@ServerName) + '.' + QUOTENAME(DB_NAME()) + 'Script to setup reporting';
GO
...
BEGIN
    ...
    ELSE
    BEGIN
        EXEC @return_value = [dbo].[spMFDropAndUpdateMetadata] @IsReset = 0                               -- smallint
                                                              ,@ProcessBatch_ID = @ProcessBatch_ID OUTPUT -- int
                                                              ,@Debug = 0                                 -- smallint
...
IF EXISTS (SELECT 1 FROM msdb.dbo.sysmail_account a) AND @EDIT_MAILPROFILE_PROP <> '{varEmailProfile}'
BEGIN
