...
EXEC setup.[spMFSQLObjectsControl] @SchemaName = N'dbo', @ObjectName = N'spMFGetObjectvers', -- nvarchar(100)
    @Object_Release = '4.3.10.47', -- varchar(50)
    @UpdateFlag = 2 -- smallint
...
SELECT @ClassId = MFID FROM MFClass WHERE TableName = @TableName
  ...
  SET @StartTime  = GETUTCDATE();
                            ...
                            SET @LogTextDetail = 'Objver with filters: Date: ' + CAST(ISNULL(@dtModifiedDate,'2000-01-01') AS NVARCHAR(30)) + ' Objids: ' + ISNULL(@MFIDs,'')
...
EXEC [Setup].[spMFSQLObjectsControl] @SchemaName = N'dbo'
                                    ...
                                    ,@Object_Release = '4.8.10.49'           -- varchar(50)
                                    ,@UpdateFlag = 2;
-- smallint
GO
...
EXEC [dbo].[spMFUpdateMFilesToMFSQL] @MFTableName = @MFTableName  -- nvarchar(128)
				...
				--EXEC [dbo].[spMFUpdateTableWithLastModifiedDate] @UpdateMethod = @UpdateMethod -- int
...
WHEN ISNULL(t.id,0) > 0 AND ISNULL(t.[objid],0) = 0 THEN 6
END 
FROM [#AllObjects] AS [ao]
...
ON ao.[objid] = t.[objid] ;';
			...
			SELECT @ToUpdateObjver = COUNT(*) FROM [#AllObjects] AS [ao] WHERE [ao].[StatusFlag] IN (1,5)
			                           ...
			                           SET @LogTypeDetail = 'Status';
			                           SET @LogStatusDetail = 'In progress';
			                           SET @LogTextDetail = 'Objvers: Total: '+ CAST(COALESCE(@TotalObjver,0) AS NVARCHAR(10)) + ' Filtered: ' + CAST(COALESCE(@FilteredObjver,0) AS NVARCHAR(10)) + ' To update: ' + CAST(COALESCE(@ToUpdateObjver,0) AS NVARCHAR(10))
...
*************************************************************************************/
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    -------------------------------------------------------------
                    -- class table update with no filters
                    -------------------------------------------------------------
