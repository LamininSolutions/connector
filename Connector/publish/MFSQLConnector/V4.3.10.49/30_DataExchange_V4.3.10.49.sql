...
--	RAISERROR('This procedure is suspended awaiting a bug fix',16,1)
  ...
  IF @debug > 0     SELECT @RecCount AS [RecCount];

        SELECT @BatchSize = 4000 / (LEN(@ToObjid)+1);

     IF @debug > 0     SELECT @BatchSize AS [MAxBatchsize];
       ...
       IF @debug > 0        SELECT @BatchesToRun AS BatchestoRun;
        ...
        BEGIN
            ...
            SET @objids = NULL;
            SET @Message
                = 'Batch: ' + CAST(ISNULL(@BatchCount,1) AS VARCHAR(10)) + ' Started: ' + CAST(@StartTime AS VARCHAR(30));
		   ...
		   SET @ObjIdCount = CASE
            ...
            SET @SQL
                = N'(SELECT TOP ' + CAST(@BatchSize AS VARCHAR(12)) + ' identity(int,'
       ...
       IF @debug > 0     SELECT @SQL AS SQL;
            ...
            SET @StartRow = CASE WHEN @StartRow > @MaxRow THEN NULL 
			ELSE (SELECT MAX(Objid) FROM ##ObjidTable) + 1 end
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            SET @ProcessingTime = DATEDIFF(MILLISECOND, @StartTime, GETDATE());
            SET @Message
                = CAST(@ProcessingTime / 1000 AS VARCHAR(10)) + ' Update ObjVer in table MFAuditHistory Start ';
            ...
            SET @SQL = N'
SELECT @MFLastModifiedDate = (SELECT MAX([MF_Last_Modified]) FROM ' + QUOTENAME(@MFTableName) + ' );';
            ...
            SET @ProcessingTime = DATEDIFF(MILLISECOND, @StartTime, GETDATE());
            SET @Message = CAST(@ProcessingTime / 1000 AS VARCHAR(10)) + ' Update ObjVer in table MFAuditHistory End ';
        ...
        BEGIN
            ...
            -- ebd while
            -------------------------------------------------------------
            -- updating table audit after batch processing has ended
            -------------------------------------------------------------
            --SET @ProcessingTime = DATEDIFF(MILLISECOND, @StartTime, GETDATE());
            --SET @Message = CAST(@ProcessingTime / 1000 AS VARCHAR(10)) + ' Update ObjVer in table MFAuditHistory Start ';
...
ON t.id = t2.id';
                ...
                SET @Message
                    = 'Batch: ' + CAST(@BatchCount AS VARCHAR(10)) + ' Processing (s) : '
                      + CAST(@ProcessingTime / 1000 AS VARCHAR(10)) + ' remaining: '
