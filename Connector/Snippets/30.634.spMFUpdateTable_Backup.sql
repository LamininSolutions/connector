...
BEGIN
	...
	*/
                    ...
                    BEGIN
                        ...
                        BEGIN
                            SET @ObjVerXMLForUpdate
                                = REPLACE(
                                             CAST(@ObjVerXMLForUpdate AS NVARCHAR(MAX)),
                                             '</form>',
                                             @missingXML + '</form>'
                                         );
                        END;
			...
			IF @Debug = 99
			BEGIN
            SELECT CAST(@XML AS NVARCHAR(MAX)), CAST(@ObjVerXmlString AS NVARCHAR(MAX)) ,CAST(@MFIDs AS NVARCHAR(MAX)),CAST(@MFModifiedDate AS NVARCHAR(MAX))
        ...
        BEGIN
            ...
            BEGIN
                IF @ErrorInfoCount > 0
                    SET @LogStatus = N'Partial Successful';
                SET @LogText
                    = @LogText + ':' + @ProcedureStep + 'with M-Files errors: ' + @TableName + 'Return Value 3';
                SET @LogType = CASE
