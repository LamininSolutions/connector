...
PRINT SPACE(5) + QUOTENAME(@@SERVERNAME) + '.' + QUOTENAME(DB_NAME()) + '.[dbo].[procname]';
GO
SET NOCOUNT ON
EXEC [setup].[spMFSQLObjectsControl]
	@SchemaName = N'dbo'
  , @ObjectName = N'procname' -- nvarchar(100)
  , @Object_Release = '3.1.x.xx'
  , @UpdateFlag = 2
...
/*------------------------------------------------------------------------------------------------
	...
	PARAMETERS:
			...
			@Param2:				Optional - 
			@Param3:				Optional - Default(0) - Summary Only
															1  	- x
															2	- y
...
/*
Adding aliases
Prefix + Name of property (Excluding SpecialCharacters & spaces)
*/

DECLARE @MFTableNames NVARCHAR(400); --options Property , Class, ObjectType, Valuelist, ValuelistItem
DECLARE @Prefix NVARCHAR(10); --constant prefer before name
DECLARE @WithUpdate BIT = 0;
...
SET @MFTableNames = 'MFProperty,MFClass';
SET @Prefix = 'LSConnect';
SET @WithUpdate = 0;
SET @Debug = 1;
    ...
    (
        ...
        [Metadata]
            ...
            FROM
				...
				IF @debug > 0
				SELECT * FROM @MetadataList AS [ml];
...
IF @Debug > 0
    BEGIN
        SELECT
            @ValidTableNamesList ;
        SELECT
                'invalid' AS invalids, [ml].[TableName]
        FROM
                @MetadataList    AS [ml]
            LEFT JOIN
                @ValidTableNames AS [vtn]
...
IF EXISTS
    ...
    BEGIN
        SET @Error = '' + @MFTableNames + ' ; Use one or more of the following list: ' + @ValidTableNamesList;
        RAISERROR('Invalid TableNames in:%s', 10, 1, @Error);
	RETURN 0
    END;
	...
	SELECT @Rowid = MIN(ID) FROM @MetadataList AS [ml]


While @Rowid IS NOT null
    BEGIN

	SELECT @TableName = Tablename, @MetadataName = Metadata FROM @MetadataList AS [ml] WHERE id = @rowID

	SET @Params = N'@Prefix nvarchar(10)'
	SET @SQL = N'
        UPDATE
            [TB]
        SET
            [Alias] = @Prefix + ''.'' + [dbo].[fnMFReplaceSpecialCharacter]([TB].[Name])
        FROM
            [dbo].'+QUOTENAME(@Tablename)+' AS [TB]
        WHERE
            [Alias] = '';'''
		EXEC sp_executesql @stmt = @SQL, @param = @Params, @Prefix = @Prefix
...
EXEC [dbo].[spMFSynchronizeSpecificMetadata]
	...
	SELECT @Rowid = MIN(id) FROM @MetadataList AS [ml] WHERE id > @RowID
	END -- end else main routine
