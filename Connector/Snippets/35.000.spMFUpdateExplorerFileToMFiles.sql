...
--                END;
                ...
                --------------------------------------------------
                --SELECT @FileLocation =  @SourcefileLocationColumn

                --SET @DebugText = ' ' + @FileLocation;
                --SET @DebugText = @DefaultDebugText + @DebugText;
                --SET @ProcedureStep = 'Get file  Location column: ';
 ...
 --               WHILE @Counter IS NOT NULL
                BEGIN
					...
					SET @params = N'@ObjIDs nvarchar(4000) output, @SQLID int'
					SET @SQL = N'Select @ObjIDs = CAST(Objid AS VARCHAR(4000)) FROM '+QUOTENAME(@MFTableName) + ' WHERE ID = @SQLID'

					EXEC sp_executeSQL @SQL, @Params, @Objids OUTPUT, @SQLID
			...
			Begin
				SET @Params = N'@Objids nvarchar(4000)'
					SET @SQL = N'UPDATE '+ QUOTENAME(@MFTableName)+' SET [Process_ID] = 1 WHERE objid = CAST(@Objids AS int)'
					EXEC sp_executeSQL @SQL, @Params, @Objids 
			...
			END
                    ...
                    --SET @Params = N'@FileID nvarchar(250) output, @SQLID int';
                    --SET @Sql = N'SELECT @FileID = [FileUniqueID] FROM ' + @TempFile + ' WHERE [RowID] = @SQLID;';

                    --EXEC [sys].[sp_executesql] @Sql, @Params, @FileID OUTPUT, @SQLID;
					...
					--SET @params = '@FileNameExist nvarchar(250) output, @SQLID int'
					--SET @SQL = N'
					...
					--EXEC sp_executeSQL @SQL, @Params, @FileNameExist OUTPUT, @SQLID
                    ...
                    SET @TableWhereClause
                        = 'y.ID = '+CAST(@SQLId AS VARCHAR(20))+'  and Process_Id= ' + CAST(@Process_ID AS VARCHAR(5));
				  ...
				  Begin
                    ...
                    SET @Sql = N'
                    Select @SQLID = (SELECT MIN(RowID) FROM ' + @TempFile + ' WHERE Rowid > @SQLID);';

                    EXEC [sys].[sp_executesql] @Sql, N'@SQLID int output', @SQLID OUTPUT;
                END; -- end loop
