...
-- VARIABLES: DEBUGGING
-------------------------------------------------------------
DECLARE @ProcedureName AS NVARCHAR(128) = N'spMFUpdateClassAndProperties';
...
BEGIN TRY
    ...
    SET @ProcedureStep = N'Table Exists';
    ...
    BEGIN
        SELECT @ProcedureStep = N'Get Security Variables';
        ...
        ------------------------------------------------------			
        SET @ProcedureStep = N'Check Objid exists';

        SELECT @RowExistQuery
            = N'SELECT @retvalOUT  = COUNT(ID) FROM ' + @MFTableName + N' WHERE objID ='
              + CAST(@ObjectID AS NVARCHAR(10));

        SELECT @Definition = N'@retvalOUT int OUTPUT';

        EXEC sys.sp_executesql @RowExistQuery,
            @Definition,
            @retvalOUT = @count OUTPUT;

        SET @LogTypeDetail = N'Status';
        SET @LogStatusDetail = N'Check ObjectID';
        SET @LogTextDetail = N'Objid: ' + CAST(@ObjectID AS VARCHAR(10));
        ...
        BEGIN --object exists
            ...
            --To Get Table Name
            ------------------------------------------------------
            SELECT @ProcedureStep = N'Reset Table name';
            ...
            --Set Object Type Id
            ------------------------------------------------------
            SELECT @ProcedureStep = N'Get Object Type and Class';

            -------------------------------------------------------------
            -- Is class change
            -------------------------------------------------------------
            SET @ProcedureStep = N'Class or Columname exist';
            ...
            BEGIN
                ...
                SELECT @ProcedureStep = N'Convert Values to Column Value Table';
                ...
                SELECT @RecordDetailsQuery
                    = N'SELECT @objID = [OBJID] ,@ObjVersion = [MFVersion] FROM ' + @MFTableName + N' WHERE [objID] = '
                      + CAST(@ObjectID AS NVARCHAR(20)) + N'';
                ...
                EXEC sys.sp_executesql @RecordDetailsQuery,
                    @ObjIdOut,
                    @objID = @objID OUTPUT,
                ...
                --------------------------------------------------------------------------- 
                SET @ProcedureStep = N'Generate Query';
                ...
                SELECT @ProcedureStep = N'Add new columns';
                ...
                SELECT @ProcedureStep = N'Inserting values from @NewPropertyValues_Table into @ColumnValuePair';
                ...
                -- CREATING XML
                ------------------------------------------------------
                SELECT @ProcedureStep = N'Generate XML File';
                ...
                SELECT @XML = N'<form>' + (CAST(@FullXml AS NVARCHAR(MAX))) + N'</form>';
                ...
                SELECT @ProcedureStep = N'Wrapper Method spMFCreateObjectInternal ';
                ...
                SET @LogColumnName = N'MFUpdateHistory: ObjectVerDetails';
                ...
                DECLARE @CheckOutObjects NVARCHAR(MAX);

                ------------------------------------------------------
                -- CALLING WRAPPER METHOD
                ------------------------------------------------------
                EXECUTE dbo.spMFCreateObjectInternal @VaultSettings,
                    @XML,
                    NULL,
                    @MFIDs,
                    0,
                ...
                SET @ProcedureStep = N'Updating MFTable with ObjID and MFVersion';
			...
			--		,Deleted = 0
                ...
                ----------------------------------------------------------------------------------------------------------
                SET @ProcedureStep = N'Updating MFTable with Process_ID = 2,if any synch error occurs';
                ...
                BEGIN
					...
					,LastModified = GETDATE()
					FROM #SynchErrObjVer
					WHERE ['                   + @MFTableName + N'].ObjID = #SynchErrObjVer.ObjID';
                    ...
                    -- LOGGING THE ERROR
                    ------------------------------------------------------
                    SELECT @ProcedureStep = N'Update MFUpdateLog for Sync error objects';
                ...
                SELECT @ProcedureStep = N'Updating MFTable with ObjID and MFVersion';
  ...
  IF @Debug > 0
                ...
                */
                ...
                BEGIN
                    ...
                    SELECT @ProcedureStep = N'Select Table Name from NEW CLASS';
                    ...
                    BEGIN
                        SET @ProcedureName = N'spMFUpdateTableInternal';
                        SET @ProcedureStep = N'Update property details from M-Files in new Class Table ';
                ...
                SET @LogTextDetail = N'Insert record for table ' + @MFTableName;
                SET @LogColumnName = N'';
                SET @LogColumnValue = N'';
            ...
            BEGIN
                ...
                SET @DebugText = N'';
                SET @ProcedureStep = N'Missing Parameters ';
                SET @LogStatus = N'Error';
                SET @LogTextDetail = N'Either ClassID or ColumnNames or both must be used ';
        ...
        BEGIN
            SET @DebugText = N'';
            SET @DebugText = @DefaultDebugText + @DebugText;
            SET @ProcedureStep = N'Incorrect Object ';
            SET @LogStatus = N'Error';
            SET @LogTextDetail = N'Object ID is invalid ';
    ...
    BEGIN
        SET @DebugText = N'';
        SET @DebugText = @DefaultDebugText + @DebugText;
        SET @ProcedureStep = N'Check Tablename ';
        SET @LogStatus = N'Error';
        SET @LogTextDetail = N'Tablename does not exist';
